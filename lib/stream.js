...
function Server(stream, fns, fnResolver) {
	var self = this;

	self.fns = fns || {};
	self.fnResolver = fnResolver;
	self.stream = stream;
	...
	self.stream.on('end', function() {
	    // If our client closed the connection, let's just emit an end as well
	    self.emit('end');
	});
}
...
Server.prototype._doCall = function(fn, m) {
	...
	if(m.isNotification()) {
		...
		var resolvedFunction = function(err, varargs) {
			// TODO, all of the below code up to self._write can be deleted once we
			// can use a rest operator (nodejs 6?)
...
Server.prototype._call = function(m) {
	...
	Q.Promise(function(resolve, reject) {
		if(typeof self.fns[m.obj.method] === 'function') {
			return resolve(self.fns[m.obj.method]);
		}
		self.fnResolver(m.obj.method, function(err, result) {
