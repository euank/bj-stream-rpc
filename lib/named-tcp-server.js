...
//	fnResolver: function(string) -> (error, fn), // Function to resolve unrecognized functions with
// }
function Server(host, port, fns, options, callback) {
	...
	self.servers = {};
	self.fns = fns;
	self.fnResolver = options.fnResolver;
	...
	self.server = net.createServer(function(conn) {
		...
		var server = new NamedStreamServer(conn, self.fns, self.fnResolver);
